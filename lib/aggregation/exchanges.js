/* eslint-disable camelcase,no-continue,no-param-reassign,@typescript-eslint/no-this-alias,func-names,prefer-template,object-shorthand,prefer-destructuring,import/no-unresolved,import/extensions,@typescript-eslint/no-var-requires,no-var */
...
function ExchangeAggregation(options) {
  ...
  this.purge = setInterval(function () {
    ...
    const minute = moment.utc().startOf('hour').subtract(1, 'hour')
    const hour = moment.utc().startOf('day').subtract(1, 'day')
    const day = moment.utc().startOf('month').subtract(1, 'month')
    const month = moment.utc().startOf('year').subtract(1, 'year')

    // remove cached minutes
    // eslint-disable-next-line no-restricted-syntax
    for (const time in self.cached.minute) {
    ...
    // eslint-disable-next-line no-restricted-syntax
    for (const time in self.cached.month) {
  ...
  function aggregate(force) {
    ...
    function aggregateMinutes(_force) {
      ...
      function aggregateUpdated() {
        ...
        for (const time in self.cached.minute) {
          if (self.cached.minute[time].updated) {
            const key = '1minute|' + self.keyBase + '|' + utils.formatTime(time)
            const reduced = self._reduce(self.cached.minute[time].exchanges)
            ...
            if (reduced) {
              ...
              // this can be the case if the amounts are too small to be counted
              self.log.warn('NOT REDUCED (too small?)', key, JSON.stringify(self.cached.minute[time]))
            }
          }
        }
      }
    ...
    function aggregateInterval(multiple, period, interval) {
      ...
      for (const key in intervalTable) {
      ...
      // eslint-disable-next-line no-restricted-syntax,guard-for-in
      for (time in times) {
        const start = moment.utc(times[time])
        time = times[time]
        const end = moment.utc(time).add(multiple, period)

        const intervals = {}
        while (end.diff(time) > 0) {
          const row = self.cached[interval][time.format()]
        ...
        const key = multiple + period + '|' + self.keyBase + '|' + utils.formatTime(start)

        // self.log.info(key, table);
        const reduced = self._reduce(intervals, period)
        ...
        if (reduced) {
          ...
          // this can be the case if the amounts are too small to be counted
          self.log.warn('NOT REDUCED', table, key)
        }
      }
    }
    ...
    function update() {
      ...
      for (const table in updated) {
        // eslint-disable-next-line no-restricted-syntax,guard-for-in
        for (const rowkey in updated[table]) {
...
ExchangeAggregation.prototype._reduce = function (rows, period) {
  ...
  for (key in rows) {
    ...
    if (!period) {
      if (this.base.currency === 'XRP' && rows[key].base_volume <= 0.0005) {
        continue
      } else if (this.counter.currency === 'XRP' && rows[key].counter_volume <= 0.0005) {
        continue
      }
    }
