...
class Log {
  ...
  constructor(options) {
    ...
    if (options.file) {
      this.transports.push(new winston.transports.File({ filename: options.file }))
    ...
    if (!options.disableConsoleLog) {
      this.transports.push(new winston.transports.Console({ level: 'debug' }))
    }
  ...
  log = (type, args) => {
    ...
    args.push('')
    this.logger[type].apply(null, args)
  }
