...
var Validator = function () {
  ...
  var working
  var timer
  var startIndex = config.get('startIndex')

  if (startIndex && startIndex < GENESIS_LEDGER) {
    startIndex = GENESIS_LEDGER - 1
  }

  this.importer = importer // need this for storm
  this.start = function () {
    if (!timer && !startIndex) {
  ...
  function checkNextLedger() {
    hbase.getLedger(
      ...
      function (err, ledger) {
        ...
        if (err && err.toString().indexOf('missing transaction') !== -1) {
        ...
        if (!ledger) {
          ...
          for (var i = 0; i < ledger.transactions.length; i++) {
            if (
              ledger.transactions[i].ledger_index !== ledger.ledger_index ||
        ...
        if (hash !== ledger.ledger_hash) {
          message =
            'ledger does not hash to the expected value ' +
            'ledger_index: ' +
            ledger.ledger_index +
            '\n' +
            'actual ledger hash: ' +
  ...
  function notify(ledger_index, message) {
    ...
    if (!notifications[ledger_index]) {
      ...
      var params = {
        ...
        to: to,
        subject: name + ' - Validation error',
        html:
          'The validation process encountered an unexpected error: <br /><br />\n' +
          '<blockquote>' +
          message +
          '</blockquote><br />\n' +
          'https://data-staging.ripple.com/v2/ledgers/' +
          ledger_index +
          ' <br />\n' +
  ...
  function convertLedger(ledger) {
    return {
      ...
      closeTime: moment.unix(ledger.close_time).utc().format('YYYY-MM-DDTHH:mm:ss.SSS[Z]'),
      ...
      parentCloseTime: moment.unix(ledger.parent_close_time).utc().format('YYYY-MM-DDTHH:mm:ss.SSS[Z]'),
