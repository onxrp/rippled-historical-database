...
var Importer = function() {
  ...
  this.api = new xrpl.Client(config.get('ripple').server)
  this.api.connect()
  ...
  */
  var BackFiller = function (stopIndex, startIndex, callback) {
    ...
    self.api.connect().then(function() {
    ...
    function advanceQueue () {
      ...
      while (1) {
        ...
        if (queue[index] === 'pending') {
          ...
          if (earliestParentHash && earliestParentHash != queue[index].ledger_hash && earliestParentHash != queue[index].parent_hash) {
  ...
  */
  var LiveStream = function () {
    ...
    self.api.connect();
    self.api.on('ledger', function(resp) {
      if (!self._active) return;
      log.info('['+new Date().toISOString()+']', 'ledger closed:', resp.ledger_index);
      getValidatedLedger(resp.ledger_index);
    });
    ...
    function handleLedger(ledger) {
      ...
      var current = ledger.ledger_index;
  ...
  self.getLedger = function (options, callback) {
    ...
    function requestLedger(options, callback) {
      ...
      log.info('['+new Date().toISOString()+']', 'requesting ledger:', options.ledger_index);

      self.api.ledger(options)
      .then(processLedger)
      .catch(function(e) {
        log.error("error requesting ledger:", options.ledger_index, e);
      ...
      function processLedger(ledger) {
        ...
        if (!options.expand || !options.transactions) {
        ...
        // try {
        //   hash = self.api.computeLedgerHash(ledger);

        // } catch (err) {
        //   log.error("Error calculating ledger hash: ", ledger.ledger_index, err);
        //   hashErrorLog.error(ledger.ledger_index, err.toString());
        //   //callback('unable to validate ledger: ' + ledger.ledger_index);
        //   //return;
        // }
        ...
        log.info('['+new Date().toISOString()+']', 'Got ledger: ' + ledger.ledger_index);
        callback(null, convertLedger(ledger));
      }
    }
  };
  ...
  function convertLedger(ledger) {
    var converted = {
      ...
      ledger_index: ledger.ledger_index.toString(),
      seqNum: ledger.ledger_index.toString(),
  ...
  */
  ...
  function retry(options, attempts, callback) {
    if (attempts >= 10) {
      log.error('failed to get ledger after ' + attempts + ' attempts:', options.ledger_index);
