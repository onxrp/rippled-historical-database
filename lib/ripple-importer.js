/* eslint-disable no-param-reassign,@typescript-eslint/no-this-alias,func-names,prefer-template,object-shorthand,prefer-destructuring,import/no-unresolved,import/extensions,@typescript-eslint/no-var-requires,no-var */
...
class Importer extends EventEmitter {
  ...
  hashErrorLog = winston.createLogger({
    transports: [
  ...
  constructor(options) {
    super(options)
    this.api.connect()
    this.api.connection._config.connectionTimeout = 3e4
  }
  ...
  LiveStream = () => {
    ...
    const getValidatedLedger = (index) => {
      var options = {
        ledgerVersion: index,
        includeAllData: true,
  ...
  getLedger = (options, callback) => {
     ...
     * requestLedger
     */
    const requestLedger = (_options, _callback) => {
      this.log.info('[' + new Date().toISOString() + ']', 'requesting ledger:', _options.ledgerVersion)

      this.api
        .getLedger(_options)
        .then((ledger) => processLedger(ledger))
        .catch((e) => {
          this.log.error('error requesting ledger:', _options.ledgerVersion, _options.ledgerHash, e)
          setImmediate(this.retry, _options, attempts, _callback)
        })

      /**
       * handleResponse
       */
      const processLedger = (ledger) => {
        var hash

        // if we didn't request transactions,
        // we can't calculate the transactions hash
        if (!_options.includeAllData || !_options.includeTransactions) {
          _callback(null, this.convertLedger(ledger))
          return
        }
